...
from IgnoreScopeDocker.core.op_result import OpError, OpResult
...
def _make_hierarchy(mask_volume_names: list[str]):
    ...
    from IgnoreScopeDocker.core.hierarchy import ContainerHierarchy
...
class TestPreflightUpdate:
    ...
    def test_no_container_returns_not_found(self, tmp_path):
        ...
        from IgnoreScopeDocker.docker.container_lifecycle import preflight_update

        config = _make_config(tmp_path=tmp_path)

        with patch(
            "IgnoreScopeDocker.docker.container_lifecycle.container_exists",
            return_value=False,
        ):
...
class TestExecuteUpdate:
    ...
    def _patch_all(self, old_masks, new_masks, compose_down_ok=True, prune_ok=True):
        ...
        patches = {
            "load_config": patch(
                "IgnoreScopeDocker.docker.container_lifecycle.load_config",
                return_value=old_config,
            ),
            "compute_hierarchy": patch(
                "IgnoreScopeDocker.core.hierarchy.compute_container_hierarchy",
            ...
            "is_docker_running": patch(
                "IgnoreScopeDocker.docker.container_lifecycle.is_docker_running",
                return_value=(True, "ok"),
            ),
            "remove_container_compose": patch(
                "IgnoreScopeDocker.docker.container_lifecycle.remove_container_compose",
                return_value=(compose_down_ok, "ok" if compose_down_ok else "compose down failed", []),
            ),
            "generate_compose": patch(
                "IgnoreScopeDocker.docker.container_lifecycle.generate_compose_with_masks",
                return_value="version: '3'\nservices:\n  claude:\n    image: test\n",
            ),
            "generate_dockerfile": patch(
                "IgnoreScopeDocker.docker.container_lifecycle.generate_dockerfile",
                return_value="FROM ubuntu:22.04\n",
            ),
            "build_image": patch(
                "IgnoreScopeDocker.docker.container_lifecycle.build_image",
                return_value=(True, "built"),
            ),
            "create_container_compose": patch(
                "IgnoreScopeDocker.docker.container_lifecycle.create_container_compose",
                return_value=(True, "created", "test-container"),
            ),
            "volume_exists": patch(
                "IgnoreScopeDocker.docker.container_lifecycle.volume_exists",
                return_value=True,
            ),
            "remove_volume": patch(
                "IgnoreScopeDocker.docker.container_lifecycle.remove_volume",
                return_value=(prune_ok, "removed" if prune_ok else "in use"),
            ),
            "ensure_container_running": patch(
                "IgnoreScopeDocker.docker.container_lifecycle.ensure_container_running",
                return_value=(True, "running"),
            ),
            "ensure_container_directories": patch(
                "IgnoreScopeDocker.docker.container_lifecycle.ensure_container_directories",
                return_value=(True, "dirs ok"),
            ),
            "save_config": patch(
                "IgnoreScopeDocker.docker.container_lifecycle.save_config",
            ),
            "get_container_dir": patch(
                "IgnoreScopeDocker.docker.container_lifecycle.get_container_dir",
            ),
        }
        return patches
    ...
    def _run_update(self, patches, tmp_path):
        ...
        from IgnoreScopeDocker.docker.container_lifecycle import execute_update
