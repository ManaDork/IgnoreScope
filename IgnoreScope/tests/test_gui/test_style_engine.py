...
class TestStyleGuiAPI:
    ...
    def test_palette_color(self, sg):
        ...
        assert sg.palette_color("base_0") == "#0F0A1E"

    def test_ui_color(self, sg):
        assert sg.ui_color("window_bg") == "#1A1035"
        assert sg.ui_color("accent_primary") == "#8B5CF6"
    ...
    def test_build_stylesheet(self, sg):
        ...
        assert len(css) > 100
        assert "#1A1035" in css  # window_bg
        assert "#8B5CF6" in css  # accent_primary
...
class TestResolveGradientColor:
    ...
    def test_palette_lookup(self, sg):
        assert sg._resolve_gradient_color("base_0") == "#0F0A1E"

    def test_ui_lookup(self, sg):
        assert sg._resolve_gradient_color("surface_bg") == "#2D1F55"

    def test_unknown_fallback(self, sg):
        result = sg._resolve_gradient_color("nonexistent_var")
        assert result == "#0F0A1E"  # falls back to base_0
...
class TestBuildWidgetGradient:
    ...
    def test_stop_colors_resolved(self, sg):
        ...
        grad = sg.build_widget_gradient("main_window", 800, 600)
        stops = grad.stops()
        # base_1 = #1A1035, base_0 = #0F0A1E
        assert_color(stops[0][1], "#1A1035", "stop 0 = base_1")
        assert_color(stops[1][1], "#0F0A1E", "stop 1 = base_0")
