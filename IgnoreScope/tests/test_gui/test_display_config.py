...
class TestFolderTruthTable:
    ...
    def test_folder_masked_revealed(self):
        ...
        assert resolve_tree_state(ns, is_folder=True) == "FOLDER_VIRTUAL_REVEALED"

    def test_folder_masked_mirrored(self):
        ns = NodeState(visibility="virtual", has_direct_visible_child=False)
        assert resolve_tree_state(ns, is_folder=True) == "FOLDER_VIRTUAL"
...
class TestStateStylesDict:
    ...
    def test_all_states_present(self, config):
        expected = {
            ...
            "FOLDER_VIRTUAL_REVEALED", "FOLDER_VIRTUAL",
    ...
    def test_folder_virtual_revealed_gradient(self, config):
        """F5: vis=virtual, desc=revealed."""
        style = config.state_styles["FOLDER_VIRTUAL_REVEALED"]
        g = style.gradient
        assert (g.pos1, g.pos2, g.pos3, g.pos4) == (
            "virtual", "virtual", "revealed", "revealed"
        )

    def test_folder_pushed_ancestor_gradient(self, config):
        """F8: vis=hidden, desc=pushed."""
        style = config.state_styles["FOLDER_PUSHED_ANCESTOR"]
        g = style.gradient
        assert (g.pos1, g.pos2, g.pos3, g.pos4) == (
            "background", "background", "pushed", "pushed"
        )

    def test_file_pushed_gradient(self, config):
        """FILE_PUSHED: F1=hidden(vis), F2=bg, F3=bg(sync deferred), F4=pushed."""
        style = config.state_styles["FILE_PUSHED"]
        g = style.gradient
        assert (g.pos1, g.pos2, g.pos3, g.pos4) == (
            "hidden", "background", "background", "pushed"
        )
...
class TestJsonLoading:
    ...
    def test_color_vars_count(self, config):
        """tree_state_style.json color vars."""
        assert len(config.color_vars) == 11

    def test_color_vars_keys(self, config):
        expected_keys = {
            "background", "mounted", "pushed", "masked",
            "revealed", "visible", "virtual", "hidden",
            "warning", "selected", "container_only",
        }
