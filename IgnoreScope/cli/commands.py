...
def cmd_create_from_template(
    ...
    """Create a scope config from a template JSON file.

    Loads the template, resolves paths against host_project_root,
    validates sibling paths, and saves as the scope config.

    Args:
        host_project_root: Project root directory
        template_path: Path to template JSON file
        scope_name: Scope name (default: 'dev')

    Returns:
        Tuple of (success, message)
    """
    import json
    from .._version import check_version_mismatch
    ...
    # Run migration if template is old format
    ...
    # Warn about missing sibling paths
    warnings = []
    for sibling in config.siblings:
        if not sibling.host_path.exists():
            warnings.append(f"  Sibling path not found: {sibling.host_path}")

    save_config(config)

    msg = f"Config created from template: {template_path.name}"
    if warnings:
        msg += "\nWarnings:\n" + "\n".join(warnings)
    return True, msg
