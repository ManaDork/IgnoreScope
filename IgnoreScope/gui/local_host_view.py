...
class LocalHostView(QWidget):
    ...
    def _show_header_context_menu(self, pos: QPoint) -> None:
        ...
        menu = QMenu(self)
        mount_action = QAction(f"Mount {root.name}", menu)
        mount_action.setCheckable(True)
        mount_action.setChecked(self._tree.is_in_raw_set("mounted", root))
        mount_action.triggered.connect(
            lambda checked: self._tree.toggle_mounted(root, checked),
        )
