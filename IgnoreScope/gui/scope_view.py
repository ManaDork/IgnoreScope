"""Scope View — right panel showing container file view with Push column.

ScopeView: QWidget wrapping a QTreeView + MountDataTreeModel with
ScopeDisplayConfig. Shows files and folders with Push (display-only) column.
All file operations via RMB context menu + signal emission.
...
class ScopeView(QWidget):
    """Right panel: container view with Push column.
    ...
    def refresh(self) -> None:
        ...
        # Dynamic column 1 visibility (hide when container can't accept pushes)
        if self._config.columns[1].visible != show_pushed:
            self._config.columns[1] = dc_replace(
                self._config.columns[1], visible=show_pushed,
            )
