...
def generate_container_structure(
    ...
    its effective visibility: [visible], [virtual], or [hidden].
...
def _has_revealed_descendant(path: Path, revealed: set[Path]) -> bool:
    ...
    NOTE: This duplicates logic from MountSpecPath.has_exception_descendant()
    but operates on flat path sets (legacy API) rather than pattern strings.
    This module walks the filesystem live and doesn't have MountSpecPath context.
    See pathspec-native-state feature for the CORE-authoritative version.
    """
