...
## Container Mode Terms

Per-scope delivery-mode switch introduced in 0.5 to support running a Claude container against a detached snapshot of the scoped content rather than live bind-mounts. Both modes share `mount_specs` + `pushed_files` config — the difference is how scope content reaches the container at create time.

### container_mode

Runtime switch on `ScopeDockerConfig` selecting delivery mechanism. Per-scope: one project can have scopes in different modes.

**JSON Field:** `container_mode: "Hybrid" | "Isolation"` (default `"Hybrid"` for backward compatibility with pre-0.5 configs)
**Read by:** compose generation (which volumes to emit), container lifecycle (which init flow to run)

**Domains:** Config, Generation, Lifecycle

---

### Hybrid Container

The legacy delivery mode: live bind-mounts + named mask volumes at `docker compose up` time. The container sees the host's content in real time; mask/reveal layering is enforced by the kernel via Docker's volume stack. Edits on the host appear in the container immediately.

**Init surface:** compose YAML with bind mounts, mask volumes, reveal bind-mounts, isolation volumes, auth volume.
**Trade-off:** live linkage — edits flow both directions; container state is a view of host state.

**Domains:** Generation (compose.py), Lifecycle (container_lifecycle.py Hybrid branch)

---

### Isolation Container

Detached-snapshot delivery mode: no bind-mounts for project content. At create time, the container's `container_root` starts empty; content is delivered imperatively (cp or clone) and persists in the container's writable layer.

**Init surface:** compose YAML with isolation volumes + auth volume only. Project-content volumes are skipped. An init step populates the container after `compose up`.
**Trade-off:** detached — host edits after create do NOT affect container content. Enables experimental branches and AI-driven exploration without host churn.
**Piggyback:** derives its visible set from the same hierarchy filter Hybrid uses. No parallel visibility computation.

**Domains:** Generation (compose.py Isolation branch), Lifecycle (container_lifecycle.py Isolation init)

---

### init_source

How an Isolation container's content is seeded after `compose up`. Read only when `container_mode == "Isolation"`.

**Values:**
- `"cp"` — walk the scoped visibility set and `docker cp` each host entry into the container. Phase 1 default.
- `"clone"` — `docker exec git clone` a VCS remote into `container_root`. Phase 3+ (not yet shipped).

**Domains:** Config, Lifecycle

---

### mode-branching

Pattern used in compose generation and container lifecycle: a single function or pipeline reads `container_mode` and selects which outputs to emit or which init flow to run. Keeps Hybrid and Isolation paths in the same file (no fork) so sibling-handling, auth volume wiring, and extension reconciliation remain shared.

**Where it lives:** `compose.py` (volume emission), `container_lifecycle.py` (init dispatch).

**Domains:** Generation, Lifecycle

---

### Isolation symlink placeholder (guidance)

Isolation init uses a conservative policy for host symlinks and Windows junctions inside a scoped mount: create the container-side directory stub at the symlink's location, but do not traverse into the symlink target. The directory appears in the container's listing so downstream tooling sees the expected structure; populating it is left to the user via manual `docker cp` / `scope push` if needed.
...
**Domains:** Lifecycle (Isolation init), Presentation (future deferred-style state for symlink nodes)

---

### Container Mode header cue

The Project Root Header in the GUI tints to indicate the active scope's container mode. Hybrid uses the `config.mount` theme color (consistent with mount-checkbox coloring); Isolation uses `visibility.virtual` (consistent with container-only content tinting). The cue is per-scope — switching between scopes in different modes repaints the header.

**Where it lives:** theme stylesheet + `gui/local_host_view.py` header binding. The exact selector mechanism is an implementation detail.

**Domains:** Presentation (GUI theme), Config (container_mode read)
