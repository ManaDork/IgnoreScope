...
**NodeState Flag:** `container_only: bool`
**Visibility Value:** `"container_only"` — lowest priority, overridden by any host config flag
**Style:** Dedicated gradient color variable (`container_only`) with italic font
...
Computed property representing a node's container-side visibility. Derived in two stages: per-node MatrixState truth table, then tree-aware descendant walk.
...
**Mechanism — MatrixState truth table:** Derives visibility from the raw flag combination (Stage 1, per-node, no tree context needed):

| Value | Condition | Meaning |
|-------|-----------|---------|
| `orphaned` | pushed=T, masked=T, mounted=F, revealed=F | Stranded in mask volume, mount removed |
| `revealed` | revealed=T | Punch-through exposes content within masked area |
| `masked` | masked=T AND mounted=T | Content hidden by mask volume overlay |
| `visible` | mounted=T | Content appears in container via bind mount |
| `hidden` | (none of the above) | Not under any mount — not in container |
| `container_only` | container_only=T, mounted=F, masked=F, revealed=F | Exists in container only — discovered by scan diff |

**Evaluation order:** orphaned → revealed → masked → visible → container_only → hidden (first match wins).

**Mechanism — descendant walk:** Adds `mirrored` (Stage 2, requires tree context):

| Value | Condition | Meaning |
|-------|-----------|---------|
| `virtual` | effectively masked + has revealed descendant | Virtual structural path — content masked but directory structure required for container `mkdir` |

`virtual` describes directories whose structure must exist in the container even though their content is masked. When a masked directory has **revealed** descendants, the intermediate directory structure must be physically constructed in the container via `mkdir` — mirroring the host's folder tree.
...
Host:                          Container (mask on src/api/):
  src/                           src/
    api/                           api/          ← virtual (mkdir'd, content masked)
      internal/                      internal/   ← virtual (mkdir'd, content masked)
        secret.py                      (hidden)
      public/                        public/     ← revealed (bind mount punch-through)
        index.html                     index.html
```

**Note:** masked=T but mounted=F (stale config) falls through to "hidden", NOT "masked". The `masked` visibility requires an active mount.

**Implementation status:**
- Stage 1 (`orphaned`, `revealed`, `masked`, `visible`, `hidden`) → CORE (`core/node_state.py`) via MatrixState
- Stage 2 (`virtual`) → CORE (`core/node_state.py`), config-native queries (no tree walks). Controlled by `config.mirrored` toggle. Dual computation: config queries + inverse pattern derivation cross-reference.
...
Note: `masked=T, mounted=F` falls through to "hidden" (last row), NOT "masked". The `masked` visibility requires an active mount.
...
| False | Stage 2 skipped — masked stays `"masked"` | No mirrored mkdir — only pushed file parents get `mkdir -p` | Reveals still work (Docker bind mounts) but intermediate dirs absent |
