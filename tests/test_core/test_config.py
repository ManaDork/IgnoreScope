...
class TestScopeDockerConfig:
    ...
    def test_container_root_relative_serialization_round_trip(self, tmp_path: Path):
        """Test container_root serializes as relative ../mount and round-trips."""
        from IgnoreScope.core.config import ScopeDockerConfig
        ...
        # Should use relative syntax, not separate host_container_root field
        assert 'host_container_root' not in data
        cr = data['container_root']
        assert cr.startswith('..'), f"Expected relative syntax, got: {cr}"
    ...
    def test_serialization_round_trip(self, tmp_path: Path):
        ...
        # pushed_files should be inside local section (not top level)
        assert "pushed_files" not in data, "pushed_files should not be at top level"
...
class TestSiblingMount:
    ...
    def test_creation(self):
        ...
        assert sibling.pushed_files == set()
    ...
    def test_inherits_local_mount_config(self):
        """Test SiblingMount inherits from LocalMountConfig."""
        from IgnoreScope.core.config import SiblingMount
        from IgnoreScope.core.local_mount_config import LocalMountConfig

        assert issubclass(SiblingMount, LocalMountConfig)

    def test_inherited_validation_methods(self):
        """Test is_mounted, is_masked, is_revealed work on sibling instances."""
        from IgnoreScope.core.config import SiblingMount

        root = Path("C:/SharedLibs")
        sibling = SiblingMount(
            host_path=root,
            container_path="/shared",
            mounts={root / "common"},
            masked={root / "common" / "internal"},
            revealed={root / "common" / "internal" / "api"},
        )

        assert sibling.is_mounted(root / "common" / "src")
        assert sibling.is_masked(root / "common" / "internal" / "deep")
        assert sibling.is_revealed(root / "common" / "internal" / "api" / "v1")

    def test_pushed_files_serialization(self):
        """Test sibling pushed_files round-trip serialization."""
        from IgnoreScope.core.config import SiblingMount
        ...
        sibling = SiblingMount(
            ...
            container_path="/shared",
            mounts={root / "common"},
            pushed_files={root / "common" / "config.json"},
        )

        data = sibling.to_dict()
        assert "pushed_files" in data
        assert "common/config.json" in data["pushed_files"]

        restored = SiblingMount.from_dict(data)
        assert restored.pushed_files == sibling.pushed_files


class TestLocalMountConfig:
    """Tests for LocalMountConfig base class."""

    def test_pushed_files_field(self):
        """Test pushed_files field exists with default."""
        from IgnoreScope.core.local_mount_config import LocalMountConfig

        config = LocalMountConfig()
        assert config.pushed_files == set()

    def test_mirrored_field(self):
        """Test mirrored field exists with default True."""
        from IgnoreScope.core.local_mount_config import LocalMountConfig

        config = LocalMountConfig()
        assert config.mirrored is True

    def test_pushed_files_serialization(self, tmp_path: Path):
        """Test pushed_files round-trip through to_dict/from_dict."""
        from IgnoreScope.core.local_mount_config import LocalMountConfig

        config = LocalMountConfig(
            mounts={tmp_path / "src"},
            pushed_files={tmp_path / "src" / "config.json"},
        )

        data = config.to_dict(tmp_path)
        assert "pushed_files" in data
        assert "src/config.json" in data["pushed_files"]

        restored = LocalMountConfig.from_dict(data, tmp_path)
        assert restored.pushed_files == config.pushed_files

    def test_empty_pushed_files_omitted(self, tmp_path: Path):
        """Test empty pushed_files not serialized."""
        from IgnoreScope.core.local_mount_config import LocalMountConfig

        config = LocalMountConfig(mounts={tmp_path / "src"})
        data = config.to_dict(tmp_path)
        assert "pushed_files" not in data

    def test_bool_with_only_pushed_files(self, tmp_path: Path):
        """Test config is truthy when only pushed_files configured."""
        from IgnoreScope.core.local_mount_config import LocalMountConfig

        config = LocalMountConfig(pushed_files={tmp_path / "file.txt"})
        assert bool(config) is True

    def test_bool_empty(self):
        """Test empty config is falsy."""
        from IgnoreScope.core.local_mount_config import LocalMountConfig

        config = LocalMountConfig()
        assert bool(config) is False

    def test_clear_includes_pushed_files(self, tmp_path: Path):
        """Test clear() also clears pushed_files."""
        from IgnoreScope.core.local_mount_config import LocalMountConfig

        config = LocalMountConfig(
            mounts={tmp_path / "src"},
            pushed_files={tmp_path / "file.txt"},
        )
        config.clear()
        assert config.mounts == set()
        assert config.pushed_files == set()
...
class TestToAbsolutePaths:
    """Tests for the extracted to_absolute_paths utility."""

    def test_relative_paths(self, tmp_path: Path):
        """Test relative paths resolved against base root."""
        from IgnoreScope.utils.paths import to_absolute_paths

        result = to_absolute_paths(["src/main.py", "lib/util.py"], tmp_path)
        assert tmp_path / "src" / "main.py" in result
        assert tmp_path / "lib" / "util.py" in result

    def test_absolute_paths_passthrough(self, tmp_path: Path):
        """Test absolute paths pass through unchanged."""
        from IgnoreScope.utils.paths import to_absolute_paths

        abs_path = Path("C:/absolute/path.py")
        result = to_absolute_paths([str(abs_path)], tmp_path)
    ...
    def test_empty_list(self, tmp_path: Path):
        ...
        result = to_absolute_paths([], tmp_path)
        assert result == set()
...
class TestHierarchy:
    ...
    def test_mount_root_mask_volume(self, tmp_path: Path):
        ...
        assert ":ro" not in bind_mount, "Bind mount should not be :ro (Docker defaults to rw)"
        ...
        assert ":ro" not in reveal_mount, "Reveal should not be :ro (Docker defaults to rw)"
...
class TestPathHelpers:
    ...
    def test_get_igsc_root(self, tmp_path: Path):
        ...
        # Should be inside project as .ignore_scope/
    ...
    def test_get_container_dir(self, tmp_path: Path):
        ...
        # Should be .ignore_scope/{container}/
    ...
    def test_get_llm_dir(self, tmp_path: Path):
        ...
        # Should be .ignore_scope/{container}/.llm/{llm}/
    ...
    def test_load_save_config_in_project_path(self, tmp_path: Path):
        """Test config is saved to in-project path structure."""
        ...
        expected_path = get_container_dir(host_project_root, "mycontainer") / CONFIG_FILENAME
