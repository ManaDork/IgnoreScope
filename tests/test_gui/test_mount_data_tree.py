...
class TestLoadConfigWithSiblings:
    ...
    def test_sibling_sets_merged(self, tree, project_dir, sibling_dir):
        ...
        assert project_dir / "src" in tree._mounts
        assert sibling_dir / "common" in tree._mounts
        # Sibling-specific sets present
        assert sibling_dir / "common" / "internal" in tree._masked
        assert sibling_dir / "common" / "internal" / "api" in tree._revealed
    ...
    def test_multiple_siblings(self, tree, project_dir, sibling_dir, second_sibling_dir):
        ...
        sib1 = SiblingMount(host_path=sibling_dir, container_path="/shared")
        sib2 = SiblingMount(host_path=second_sibling_dir, container_path="/other")
...
class TestGetConfigDataExcludesSiblings:
    ...
    def test_sibling_mounts_excluded(self, tree, project_dir, sibling_dir):
        ...
        assert project_dir / "src" in data['mounts']
        assert sibling_dir / "common" not in data['mounts']
...
class TestToggleOnSiblingPath:
    ...
    def test_toggle_mask_on_sibling_path(self, tree, project_dir, sibling_dir):
        """Toggling mask on sibling path → extracted to correct sibling."""
        sibling = SiblingMount(
            host_path=sibling_dir,
            container_path="/shared",
            mounts={sibling_dir / "common"},
        ...
        tree.toggle_masked(sibling_dir / "common" / "internal", True)
